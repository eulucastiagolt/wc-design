window.addEventListener("DOMContentLoaded", ()=>{
    ...
    allBtnModal.forEach((e)=>{
        ...
        if (modal.querySelector(".wc-close-button")) modal.querySelector(".wc-close-button").addEventListener("click", ()=>{
            modal.classList.remove("wc-show-modal");
            document.querySelector("body").style.overflow = "auto";
        });
        if (modal.querySelector(".wc-close-modal")) modal.querySelector(".wc-close-modal").addEventListener("click", ()=>{
            modal.classList.remove("wc-show-modal");
            document.querySelector("body").style.overflow = "auto";
        });
        if (modal.querySelector(".wc-modal-background-fixed")) modal.querySelector(".wc-modal-background-fixed").addEventListener("click", ()=>{
            modal.classList.remove("wc-show-modal");
            document.querySelector("body").style.overflow = "auto";
        });
    });
    ...
    document.addEventListener("click", function(elem) {
        ...
        } else if (document.querySelectorAll(".wc-custom-select-options.open").length > 1) document.querySelectorAll(".wc-custom-select-options.open").forEach((e)=>{
 ...
 * keyAndValue is boolean allowing only true/false, default is false */ function wc_autocomplete(settings, object) {
    ...
    elementQuery.addEventListener("input", function() {
        ...
        if (query.length > 2 && wc_load_autocomplete(settings, object, query).length > 0) {
            ...
            wc_load_autocomplete(settings, object, query).forEach((lists)=>{
                ...
                } else contentList += `<li><a href="#">${lists.title}</a></li>`;
            });
        } else viewAutocomplete.classList.remove("show");
    });
}
function wc_load_autocomplete(settings, array, query) {
    if (settings.keyAndValue === true) return array.filter((e)=>{
        return e.title.toLowerCase().indexOf(query.toLowerCase()) > -1;
    });
    else return array.filter((e)=>{
...
const wc = function(element) {
    return {
        ...
        notify: function(options = null) {
            if (!options.eventType) options.eventType = "snapshot";
            ...
            if (!document.querySelector(".wc-all-notify")) document.querySelector("body").insertAdjacentElement("beforeend", locallAllNotify);
            if (options.eventType != "snapshot") document.querySelector(element).addEventListener("click", function() {
                document.querySelector(".wc-all-notify").appendChild(notifyElement);
            });
            else document.querySelector(".wc-all-notify").appendChild(notifyElement);
        }
    };
};

//# sourceMappingURL=index.794eb16e.js.map
